 ...
 *    AULA_ID e INTERVALO_ENVIO, utilizadas para identificar el origen de
 *    cada medición y para evitar colisiones en el canal LoRa.
 *  • ESTE ARCHIVO CORRESPONDE AL NODO DEL AULA 2. Ambos nodos comparten el
 *    mismo canal LoRa (misma frecuencia, SF, BW y Sync Word), por lo que si
 *    los dos transmitieran exactamente cada 15 segundos, sus paquetes
 *    terminarían coincidiendo en el aire una y otra vez ("colisión LoRa":
 *    el gateway recibe una mezcla de ambas señales y no logra decodificar
 *    ninguna correctamente). Para evitarlo, el nodo del Aula 2 transmite
 *    cada 17 segundos en lugar de 15: al no ser múltiplos exactos entre sí,
 *    las transmisiones de ambos nodos se van desfasando con el tiempo en
 *    vez de repetirse siempre en el mismo instante.
 *
 ******************************************************************************/
...
// IMPORTANTE: en el nodo del Aula 1 este valor es 15000 (15 s). Aqui se usa
// 17000 (17 s) a proposito, para evitar que ambos nodos transmitan siempre
// al mismo tiempo y colisionen en el canal LoRa (ver observaciones arriba).
// Si en el futuro se agregan mas nodos, cada uno deberia tener tambien un
// intervalo ligeramente distinto (ej. 15000, 17000, 19000...).
#define INTERVALO_ENVIO 17000
